...
export class ApiVideoCallService {
  ...
  httpOptions = {
    ...
    getSingleData(id: string): Observable<Message> {
      this.url = this.getBackendUrl();
      return this.http.get<SingleObject>(this.url + this.endPGetTableData + "/" + id)
      .pipe(
        retry(1),
        catchError(this.processError),
        map(ro => ro.message)
    ...
    /**add get vieo url + arguments s3_path to each items on getallitems */
    getVideo(id: string): Observable<string> {
      this.url = this.getBackendUrl();
      return this.http.get<VideoInfo>(this.url + this.endPVideoUrl + "/" + id)
      .pipe(
        retry(1),
        catchError(this.processError),
        map(ro => ro.message)
    ...
    setDescription(id: string, description: string): Observable<void> {
      this.url = this.getBackendUrl();
      return this.http.put<void>(this.url + this.endPDescription + "/" + id, {'description': description})
      .pipe(
        retry(1),
        catchError(this.processError)
        );
