...
class MSTeamsWebhookNotifier:
    ...
    def send_notification(self, message: str) -> None:
        ...
        request.add_header("Content-Length", str(len(content_bytes)))
        with urllib.request.urlopen(request, content_bytes) as response:  # nosec
            self.__logger.info(
                "webhook notifier response: %s", response.status)

    @staticmethod
    def get_body(message: str) -> dict:
        """Returns MSTeams MessageCard body

        Args:
            message (str): message to be displayed

        Returns:
            dict: MSTeams MessageCard body
        """
        return {
