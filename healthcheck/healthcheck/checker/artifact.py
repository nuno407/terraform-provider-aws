...
class BaseArtifactChecker(ABC):
    ...
    def _is_s3_anonymized_file_present_or_raise(self, file_name: str, artifact: Artifact) -> None:
        """Calls blob controller to check if anonymized file is present in S3, raises exception if not."""
        self.__blob_controller.is_s3_anonymized_file_present_or_raise(file_name, artifact)
    ...
    def _is_s3_raw_file_presence_or_raise(self, file_name: str, artifact: Artifact) -> None:
        ...
        self.__blob_controller.is_s3_raw_file_presence_or_raise(file_name, artifact)
    ...
    def _is_pipeline_execution_and_algorithm_output_doc_valid_or_raise(self, artifact: Artifact) -> list:
        """Calls db controller to check if pipeline execution and algo output documents are valid."""
        return self.__db_controller.is_pipeline_execution_and_algorithm_output_doc_valid_or_raise(artifact)
    ...
    def _is_fiftyone_entry_present_or_raise(self, artifact: Artifact, dataset: VoxelDataset) -> None:
        """Calls voxel51 controller to check if artifact entry is present in given dataset."""
        return self.__voxel_fiftyone_controller.is_fiftyone_entry_present_or_raise(artifact, dataset)
