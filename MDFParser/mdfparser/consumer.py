...
class Consumer:
    ...
    def __init__(
        ...
        self.handlers: dict[DataType, Handler] = {handler.handler_type(): handler for handler in handler_list}
    ...
    def consume_msg(self, msg: dict[str, Any]) -> Optional[OutputMessage]:
        ...
        """
        input_message = InputMessage.parse_sqs_message(msg)
        handler = self.__get_handler(input_message)
        _logger.info("Message type identified as %s", input_message.data_type.value)
    ...
    def run(self, graceful_exit: Callable[..., bool] = lambda x: x) -> None:  # type: ignore
        """
        ...
        while graceful_exit(True):
            ...
            message_body = json.loads(message.get("Body", "").replace("\'", "\""))
            output_msg = self.consume_msg(message_body)

            if output_msg:
                self.__send_message_to_metadata(output_msg)
            else:
                _logger.warning("Output message is empty and will not be sent to metadata")
