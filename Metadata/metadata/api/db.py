...
class Persistence:
    ...
    def get_signals(self, id):
        ...
        aggregation.append({'$lookup': {'from': self.__signals.name,
                           'localField': 'video_id', 'foreignField': 'recording', 'as': 'signals'}})
        result = list(self.__recordings.aggregate(aggregation))
        if (len(result) == 0):
            raise LookupError('Recording ID ' + id + ' not found')
        else:
            return result[0]
    ...
    def get_media_entry(self, recording_id):
        ...
        result = list(self.__recordings.aggregate(aggregation))
        if (len(result) == 0):
            raise LookupError('Recording ID ' + recording_id + ' not found')
        else:
            return result[0]
