...
class TestAutoMessageVisibilityIncreaser():
    ...
    @patch("base.aws.auto_message_visibility_increaser.threading.Thread")
    def test_happy_path(self, multiprocessing_process_mock):
        ...
        with AutoMessageVisibilityIncreaser(sqs_client_mock, "Receipt_handle",
                                            container_services_mock, 0, "input_queue"):
            pass
    ...
    @patch("base.aws.auto_message_visibility_increaser.threading.Thread")
    def test_unhappy_path(self, multiprocessing_process_mock):
        sqs_client_mock = Mock()
        container_services_mock = Mock()
        multiprocessing_process_mock.return_value.start = Mock()
        multiprocessing_process_mock.return_value.join = Mock()
