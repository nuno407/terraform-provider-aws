...
def main():
    ...
    METADATA_ING = MetadataIngestor(
        container_services=CS, s3_client=S3_CLIENT, sqs_client=SQS_CLIENT, sts_helper=STS_HELPER)
    SNAPSHOT_ING = SnapshotIngestor(
        container_services=CS, s3_client=S3_CLIENT, sqs_client=SQS_CLIENT, sts_helper=STS_HELPER)
    VIDEO_ING = VideoIngestor(container_services=CS, s3_client=S3_CLIENT,
    ...
    while (GRACEFUL_EXIT.continue_running):
        ...
        if message:
            ...
            if identity in VIDEO:
                ...
                if msg_obj.validate():
                    ...
                    if request_metadata:
                        ...
                        if not METADATA_ING.check_metadata_exists_and_is_complete(msg_obj):
                            ...
                            CS.update_message_visibility(
                                SQS_CLIENT, msg_obj.receipthandle, prolong_time)
                            continue
                    ...
                    if db_record_data and request_metadata:
                        ...
                        if source_data:
                            ...
                            CS.delete_message(
                                SQS_CLIENT, msg_obj.receipthandle, source)
                    ...
                    LOGGER.error(f"Message not parseable, not deleting it.", extra={
                                 "messageid": message.get('MessageId')})
                ...
                LOGGER.error(f"Could not identify message type as video nor snapshot related, skipping", extra={
                             "messageid": message.get('MessageId')})
