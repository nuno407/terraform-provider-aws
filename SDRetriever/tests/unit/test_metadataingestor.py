...
class TestMetadataIngestor:
    ...
    def test_json_raise_on_duplicates(self, obj):
        ...
        expected_result = dict(
            chunk={
                "pts_start": "25152337",
                "pts_end": "26182733",
                "utc_start": "1655453665526",
                "utc_end": "1655453676953"})
    ...
    def test_get_metadata_chunks(self, msg_interior, obj, metadata_files):
        metadata_start_time = datetime.fromtimestamp(
            msg_interior.uploadstarted /
            1000.0).replace(
            microsecond=0,
            second=0,
            minute=0)
        metadata_end_time = datetime.fromtimestamp(
            msg_interior.uploadfinished /
            1000.0).replace(
            microsecond=0,
            second=0,
            minute=0)
        ...
        obj.CS.download_file.assert_has_calls([call(ANY, obj.CS.rcc_info["s3_bucket"], file_name)
                                              for file_name in files_to_download], any_order=True)
