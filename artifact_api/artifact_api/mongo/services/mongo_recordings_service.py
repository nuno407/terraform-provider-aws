...
from base.mongo.engine import Engine, flatten_dict
...
class MongoRecordingsService():
    ...
    async def update_videos_correlations(self, correlated_videos: list[str], snapshot_ids: str | list[str]):
        """_summary_
        """
    ...
    async def update_snapshots_correlations(self, correlated_snapshots: list[str], video_ids: str | list[str]):
        """_summary_
        """
        ...
        update_snapshot = {
            "$addToSet": {
                "recording_overview.source_videos": {"$each": video_ids}
            }
        }
    ...
    async def upsert_snapshot(self, message: SnapshotArtifact, correlated_ids: list[str]):
        ...
        await self.update_snapshots_correlations([message.artifact_id], correlated_ids)
    ...
    async def upsert_video_aggregated_metadata(self, aggregated_metadata: dict[str, str | int | float | bool], correlated_id: str):
        ...
        recording_overview = DBVideoRecordingOverview.model_validate({
    ...
    async def upsert_video(self, message: S3VideoArtifact, correlated_ids: list[str]):
        ...
        overview = DBVideoRecordingOverview(devcloud_id=message.devcloudid,
                                            device_id=message.device_id,
        ...
        await self.__video_engine.update_one(
            ...
            command={
                ...
                "$set": flatten_dict(self.__snapshot_engine.dump_model(doc))
            },
            upsert=True
        )
        await self.update_videos_correlations([message.artifact_id], correlated_ids)
